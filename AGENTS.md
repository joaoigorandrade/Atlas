...
skip pruning), and the session phases (all twelve: Consume, Discriminate,
Socratic, Predict, Trace, Feynman, Perform, Drill, Connect, Crucible, Recall,
Retain) are implemented; all content is AI-generated per topic via
...
- Fonts via `next/font/google`: IM Fell English (`font.display`, titles), IM Fell English SC (`font.caps`, kickers — `font.mono` is an alias), EB Garamond (`font.serif`, body), Instrument Sans (`font.sans`, dense controls only)
...
- `lib/theme.ts` — design tokens. Never hard-code a color/font that has a token. The app is drawn as an old atlas: rag paper, iron-gall ink, engraved type. `cssVars` exposes the few tokens `globals.css` needs; everything else stays raw hex (canvas painters and `${color}33` suffixes need it).
- `components/ui/` — the shared chrome: `Masthead` (the 58px page head every sheet, the dashboard and the profile wear), `Button` (`primary` / `secondary` / `quiet`), `Plate` / `plateStyle` (a double-ruled vellum card), `SwitchMark`, and `Ornaments` (`CompassRose`, `Fleuron`, `ScaleBar`, `WaxSeal`). Reach for these before styling a header, CTA or card inline.
- Paper and textures are static on purpose (the user runs Safari, which re-runs filters and blend modes per frame): `.at-paper`, `.at-endpaper`, `.at-deckle`, `.at-dropcap`, `.at-flowerbreak` in `app/globals.css`, over images generated by `node scripts/textures.mjs` into `public/`. Never add a live `filter`, `backdrop-filter`, `mask` or `mix-blend-mode` to a large or moving surface.
- Changing screen is a page turn: `usePageTurn` (`components/atlas/usePageTurn.ts`) wraps the one screen setter in `document.startViewTransition`, and `globals.css` animates the snapshots. So sheets have no enter/exit of their own — don't reintroduce a presence lag on a screen, or the leaving page ends up in the new snapshot.
...
- Phase names (Consume, Discriminate, Socratic, Predict, Trace, Feynman,
  Perform, Drill, Connect, Crucible, Recall, Retained) are
...
Platform.dc.html` in the Claude Design project). Two things a pseudo-class
  ...
  layer** — the `at-press` / `at-lift` / `at-tint` classes that carry
